...
class KgsConnection:
        ...
        * Immediately receive response to confirm data is OK
        ...
        while not hello_received:
            ...
            if response.ok:
                ...
                raise ConnectionRefusedError("Could not connect to the KGS JSON server")
    ...
    def loop(self):
        ...
        while self._has_messages_to_process():
            ...
            message = self._dequeue_message()  # Same thing, send a single message at a time

            if message is None:
                sleep_time += 1
                if sleep_time >= self.MAX_TIME_INACTIVITY:
                    # Add a WAKE_UP message so we don't appear as idle
    ...
    def _is_processable(self, message):
        ...
        given for it, added through add_message_callback.
